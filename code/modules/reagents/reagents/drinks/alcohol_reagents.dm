...
/datum/reagent/consumable/ethanol/on_mob_life(mob/living/drinker, metabolism)
	if(iscarbon(drinker))
		var/mob/living/carbon/C = drinker
	...
	if(druggy != 0)
		drinker.set_drugginess(druggy)

	if(halluci)
		drinker.hallucination += halluci

	return ..()

/datum/reagent/consumable/ethanol/reaction_obj(obj/exposed_obj, volume)
	...
	if(istype(exposed_obj,/obj/item/book))
		if(volume > 5)
			...
			to_chat(usr, span_warning("[exposed_obj]'s ink is smeared by [name], but doesn't wash away!"))
	return ..()
