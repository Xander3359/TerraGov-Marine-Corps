...
/datum/reagent/consumable/orangejuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.getOxyLoss() && SPT_PROB(16, seconds_per_tick))
		if(affected_mob.adjustOxyLoss(-1 * 0.5 * seconds_per_tick, FALSE, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type))
...
/datum/reagent/consumable/tomatojuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.getFireLoss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 0, burn = 1 * 0.5 * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/limejuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.getToxLoss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.adjustToxLoss(-1 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/carrotjuice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_eye_blur(-2 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_temp_blindness(-2 SECONDS * 0.5 * seconds_per_tick)
	var/need_mob_update
	switch(current_cycle)
		...
		if(21 to 110)
			if(SPT_PROB(100 * (1 - (sqrt(110 - current_cycle) / 10)), seconds_per_tick))
				need_mob_update = affected_mob.adjustOrganLoss(ORGAN_SLOT_EYES, -2 * 0.5 * seconds_per_tick)
		if(110 to INFINITY)
			need_mob_update = affected_mob.adjustOrganLoss(ORGAN_SLOT_EYES, -2 * 0.5 * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/banana/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if((liver && HAS_TRAIT(liver, TRAIT_COMEDY_METABOLISM)) || is_simian(affected_mob))
		if(affected_mob.heal_bodypart_damage(brute = 1 * 0.5 * seconds_per_tick, burn = 1 * 0.5 * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/nothing/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	if(ishuman(drinker) && HAS_MIND_TRAIT(drinker, TRAIT_MIMING))
		...
		if(drinker.heal_bodypart_damage(brute = 1 * 0.5 * seconds_per_tick, burn = 1 * 0.5 * seconds_per_tick, updating_health = FALSE))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/milk/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	if(affected_mob.getBruteLoss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 1 * 0.5 * seconds_per_tick, burn = 0, updating_health = FALSE))
			. = UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/coffee/overdose_process(mob/living/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.set_jitter_if_lower(10 SECONDS * 0.5 * seconds_per_tick)

/datum/reagent/consumable/coffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_dizzy(-10 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_drowsiness(-6 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.AdjustSleeping(-40 * 0.5 * seconds_per_tick)
	//310.15 is the normal bodytemp.
	affected_mob.adjust_bodytemperature(25 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
	if(holder.has_reagent(/datum/reagent/consumable/frostoil))
		holder.remove_reagent(/datum/reagent/consumable/frostoil, 5 * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/tea/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_dizzy(-4 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_drowsiness(-2 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_jitter(-6 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.AdjustSleeping(-20 * 0.5 * seconds_per_tick)
	if(affected_mob.getToxLoss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.adjustToxLoss(-1 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			. = UPDATE_MOB_HEALTH
	affected_mob.adjust_bodytemperature(20 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/icecoffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.adjust_drowsiness(-6 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.AdjustSleeping(-40 * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	affected_mob.set_jitter_if_lower(10 SECONDS * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/hot_ice_coffee/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.adjust_dizzy(-10 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_drowsiness(-6 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.AdjustSleeping(-60 * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-7 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	affected_mob.set_jitter_if_lower(10 SECONDS * 0.5 * seconds_per_tick)
	if(affected_mob.adjustToxLoss(1 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/space_cola/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_drowsiness(-10 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/roy_rogers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	affected_mob.set_jitter_if_lower(12 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_drowsiness(-10 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	return ..()
...
/datum/reagent/consumable/nuka_cola/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.set_jitter_if_lower(40 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.set_drugginess(1 MINUTES * 0.5 * seconds_per_tick)
	affected_mob.adjust_dizzy(3 SECONDS * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/rootbeer/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.set_jitter_if_lower(4 SECONDS * 0.5 * seconds_per_tick)
	if(prob(50))
		affected_mob.adjust_dizzy(2 SECONDS * 0.5 * seconds_per_tick)
	if(current_cycle > 10)
		affected_mob.adjust_dizzy(3 SECONDS * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/spacemountainwind/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_drowsiness(-14 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.AdjustSleeping(-20 * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
	affected_mob.set_jitter_if_lower(10 SECONDS * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/dr_gibb/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_drowsiness(-12 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/space_up/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_bodytemperature(-8 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/wellcheers/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_drowsiness(3 SECONDS * 0.5 * seconds_per_tick)
	var/need_mob_update
	switch(affected_mob.mob_mood.sanity_level)
		if (SANITY_INSANE to SANITY_CRAZY)
			need_mob_update = affected_mob.adjustStaminaLoss(3 * 0.5 * seconds_per_tick, updating_stamina = FALSE)
		if (SANITY_UNSTABLE to SANITY_DISTURBED)
			affected_mob.add_mood_event("wellcheers", /datum/mood_event/wellcheers)
		if (SANITY_NEUTRAL to SANITY_GREAT)
			need_mob_update = affected_mob.adjustBruteLoss(-1.5 * 0.5 * seconds_per_tick, updating_health = FALSE)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/monkey_energy/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.set_jitter_if_lower(80 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.adjust_dizzy(2 SECONDS * 0.5 * seconds_per_tick)
	affected_mob.remove_status_effect(/datum/status_effect/drowsiness)
	affected_mob.AdjustSleeping(-40 * 0.5 * seconds_per_tick)
	affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/ice/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	if(affected_mob.adjust_bodytemperature(-5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, FALSE, affected_mob.get_body_temp_normal()))
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/soy_latte/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	affected_mob.adjust_bodytemperature(5 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
	affected_mob.set_jitter_if_lower(10 SECONDS * 0.5 * seconds_per_tick)
	if(affected_mob.getBruteLoss() && SPT_PROB(10, seconds_per_tick))
		need_mob_update += affected_mob.heal_bodypart_damage(brute = 1 * 0.5 * seconds_per_tick, burn = 0, updating_health = FALSE)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/doctor_delight/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	var/need_mob_update
	need_mob_update = affected_mob.adjustBruteLoss(-0.5 * 0.5 * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjustFireLoss(-0.5 * 0.5 * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	need_mob_update += affected_mob.adjustToxLoss(-0.5 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype)
	need_mob_update += affected_mob.adjustOxyLoss(-0.5 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type)
	if(affected_mob.nutrition && (affected_mob.nutrition - 2 > 0))
		...
		if(!(HAS_TRAIT(liver, TRAIT_MEDICAL_METABOLISM)))
			...
			affected_mob.adjust_nutrition(-2 * 0.5 * seconds_per_tick)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/cinderella/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_disgust(-5 * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/hot_coco/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.getBruteLoss() && SPT_PROB(10, seconds_per_tick))
		if(affected_mob.heal_bodypart_damage(brute = 1 * 0.5 * seconds_per_tick, burn = 0, updating_health = FALSE))
			. = UPDATE_MOB_HEALTH
	if(holder.has_reagent(/datum/reagent/consumable/capsaicin))
		holder.remove_reagent(/datum/reagent/consumable/capsaicin, 2 * 0.5 * seconds_per_tick)
	return ..() || .
...
/datum/reagent/consumable/shirley_temple/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	affected_mob.adjust_disgust(-3 * 0.5 * seconds_per_tick)
	return ..()
...
/datum/reagent/consumable/mushroom_tea/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(islizard(affected_mob))
		if(affected_mob.adjustOxyLoss(-0.5 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype, required_respiration_type = affected_respiration_type))
			return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/cucumberlemonade/on_mob_life(mob/living/carbon/doll, seconds_per_tick, times_fired)
	. = ..()
	doll.adjust_bodytemperature(-8 * 0.5 * TEMPERATURE_DAMAGE_COEFFICIENT * seconds_per_tick, doll.get_body_temp_normal())
...
/datum/reagent/consumable/mississippi_queen/on_mob_life(mob/living/carbon/drinker, seconds_per_tick, times_fired)
	...
	switch(current_cycle)
		if(11 to 21)
			drinker.adjust_dizzy(4 SECONDS * 0.5 * seconds_per_tick)
		if(21 to 31)
			if(SPT_PROB(15, seconds_per_tick))
				drinker.adjust_confusion(4 SECONDS * 0.5 * seconds_per_tick)
		if(31 to 201)
			drinker.adjust_hallucinations(60 SECONDS * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/t_letter/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.getToxLoss() && SPT_PROB(25, seconds_per_tick))
		if(affected_mob.adjustToxLoss(-2 * 0.5 * seconds_per_tick, updating_health = FALSE, required_biotype = affected_biotype))
			return UPDATE_MOB_HEALTH
