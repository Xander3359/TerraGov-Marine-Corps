	...
	switch(current_cycle)
		if(1 to 15)
			...
			if(holder.has_reagent(/datum/reagent/cryostylane))
				holder.remove_reagent(/datum/reagent/cryostylane, 5 * 0.5 * seconds_per_tick)
		if(15 to 25)
			heating = 10
		if(25 to 35)
			heating = 15
		if(35 to INFINITY)
			heating = 20
	affected_mob.adjust_bodytemperature(heating * TEMPERATURE_DAMAGE_COEFFICIENT * 0.5 * seconds_per_tick)
	...
	switch(current_cycle)
		if(1 to 15)
			...
			if(holder.has_reagent(/datum/reagent/consumable/capsaicin))
				holder.remove_reagent(/datum/reagent/consumable/capsaicin, 5 * 0.5 * seconds_per_tick)
		if(15 to 25)
			cooling = -20
		if(25 to 35)
			cooling = -30
		...
		if(35 to INFINITY)
			...
			if(prob(5))
				affected_mob.emote("shiver")
	affected_mob.adjust_bodytemperature(cooling * TEMPERATURE_DAMAGE_COEFFICIENT * 0.5 * seconds_per_tick, 50)
...
/datum/reagent/consumable/hot_ramen/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_bodytemperature(10 * TEMPERATURE_DAMAGE_COEFFICIENT * 0.5 * seconds_per_tick, 0, affected_mob.get_body_temp_normal())
...
/datum/reagent/consumable/hell_ramen/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	affected_mob.adjust_bodytemperature(10 * TEMPERATURE_DAMAGE_COEFFICIENT * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/corn_syrup/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	holder.add_reagent(/datum/reagent/consumable/sugar, 3 * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/nutriment/stabilized/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.nutrition > NUTRITION_LEVEL_FULL - 25)
		affected_mob.adjust_nutrition(-3 * 0.5 * get_nutriment_factor(affected_mob) * seconds_per_tick)
...
/datum/reagent/consumable/entpoly/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	var/need_mob_update
	if(current_cycle > 10)
		affected_mob.Unconscious(40 * 0.5 * seconds_per_tick, FALSE)
	if(SPT_PROB(10, seconds_per_tick))
		affected_mob.losebreath += 4
		affected_mob.adjustOrganLoss(ORGAN_SLOT_BRAIN, 2*0.5, 150, affected_biotype)
		affected_mob.adjustToxLoss(3*0.5, updating_health = FALSE, required_biotype = affected_biotype)
		affected_mob.adjustStaminaLoss(10*0.5, updating_stamina = FALSE, required_biotype = affected_biotype)
...
/datum/reagent/consumable/vitfro/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(55, seconds_per_tick))
		need_mob_update = affected_mob.adjustBruteLoss(-1 * 0.5 * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
		need_mob_update += affected_mob.adjustFireLoss(-1 * 0.5 * seconds_per_tick, updating_health = FALSE, required_bodytype = affected_bodytype)
	if(need_mob_update)
		return UPDATE_MOB_HEALTH
...
/datum/reagent/consumable/astrotame/overdose_process(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(affected_mob.disgust < 80)
		affected_mob.adjust_disgust(10 * 0.5 * seconds_per_tick)
...
/datum/reagent/consumable/bbqsauce
	...
	taste_multi = 2.5 //sugar's 1.5, capsacin's 1.5, so a good middle ground.
